...
annotation_col <- data.frame(Group = factor(new_normalized_data$group))

# Summarise data (there are repeated patients ids)
new_normalized_data <- new_normalized_data %>%
  group_by(id) %>%
  summarise(across(where(is.numeric), ~mean(.x, na.rm = TRUE)),  # Aggregate only numeric columns
  group = ifelse(all(is.na(group)), NA,  # If all group values are NA, return NA
        names(sort(table(na.omit(group)), decreasing = TRUE))[1]))  # Select most common non-NA group

# Check for remaining duplicates
sum(duplicated(new_normalized_data$id))  # Should return 0 if all duplicates are handled

# Assign row names according to patient id
rownames(annotation_col) <- new_normalized_data$id 
