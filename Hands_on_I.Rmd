...
Now that we have cleaned the data, we proceed to do the Exploratory Data Analysis. We used the steps provided in [this guide](https://www.kaggle.com/code/georgyzubkov/heart-disease-exploratory-data-analysis).  
...
```{r}
# 1. Check the dataset for gaps in the data
...
```{r}
# 2. Look at the distribution of the target variable: heart disease
...
```{r}
# 3: Indicate the types of variables (see paragraph above)
# 4: Look at unique values in the categorical objects
...
# 5: Analyze numerical variables (visualizing continuous columns)
...
```{r}
# 6. Look at the distribution of number of people with heart disease from various factors
...
```{r}
# 7. Look at the distribution of categorical variables depending on gender
for (var in cat_vars) {
  print(
...
```{r}
# 8. Look at outliers in numerical values depending on several factors
for (var in num_vars) {
  print(
    ggplot(data_frame, aes_string(x = "factor(num)", y = var, fill = "factor(num)")) +
      geom_boxplot(alpha = 0.7, outlier.color = "red") +
...
# Create the histogram with adjustments for styling
...
group_colors <- list(Group = c("G1" = "#F87772", "G2" = "#0FBC40", "G3" = "#639BFE", "G4" = "#9551A7"))
...
pheatmap(
  ...
  show_colnames = FALSE,  
  fontsize_row = 10,  
  main = "Cytokine Expression Heatmap",  
  annotation_col = annotation_col_group,  # Add Group annotation
  annotation_colors = group_colors  # Use custom colors
)
...
In our improved figure, we normalize the cytokine data.
...
# Merge patient gender information
aggregated_data <- left_join(aggregated_data, table_unique %>% select(id, gender), by = "id")
# Merge patient death information
aggregated_data <- left_join(aggregated_data, table_unique %>% select(id, death), by = "id")
...
normalized_data <- aggregated_data %>%
  mutate(across(-c(id, group, gender, death), min_max_norm))

# Remove unnecessary columns and prepare the data for heatmap
# Keep the first 14 columns of the data frame, except column Day (which contain the cytokine measurements we are interested in plotting) together with group, gender and death
new_normalized_data <- select(normalized_data, c(id, colnames(normalized_data)[3:14], group, gender, death))

# Create annotation data frame for G1-G4 groups, death and gender
annotation_cols <- data.frame(
  Group = factor(new_normalized_data$group),
  Death = factor(normalized_data$death, levels = c("No", "Yes")),
  Gender = factor(normalized_data$gender, levels = c("M", "F"))
)

# Define colors for the groups
group_colors <- list(
  Group = c("G1" = "#F87772", "G2" = "#0FBC40", "G3" = "#639BFE", "G4" = "#9551A7"),
  Death = c("No" = "lightblue", "Yes" = "darkblue"),
  Gender = c("M" = "gray", "F" = "pink")
)
...
# Assign row names according to patient id
rownames(annotation_cols) <- new_normalized_data$id 

# Create heatmap
# Convert to numeric matrix for heatmap, preserve patient id as row names
data_matrix <- as.matrix(new_normalized_data[,-c(1, (ncol(new_normalized_data)-2):ncol(new_normalized_data))]) # Exclude patient id and group, gender and death
